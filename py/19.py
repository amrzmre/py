...
def main():
    ...
    if not check_api_connection():
        st.error("X Cannot connect to FastAPI server. Please make sure it's running on http://localhost:8001")
            ...
            if submitted:
                if name and email:
                    ...
                    if success:
                        ...
                        st.error(f"X Error: {result.get('detail', 'Unknown error') }")
                else:
                     st.error("X Please fill in all fields")
            ...
            if selected_user_display:
                ...
                with col1:
                    ...
                    with st.form("update_user_form"):
                        ...
                        new_age = st.number_input("Age", min_value=0, max_value=120, value=selected_user['age' ])
                ...
                with col2:
                    ...
                    st.warning("A This will delete the user and all their posts!")
                    ...
                    with col2:
                        ...
                        if st.button(f"Delete", key=f"delete_post_{post['id']}", type="secondary"):
                            ...
                            if success:
                                ...
                                st.error("X Failed to delete post")    
                ...
                if success and posts:
                    ...
                    for post in posts:
                        with st.expander(f"{post['title']}"):
                            st.write(f" **Content :** {post['content']}")
                            st.write(f" **Created :** {pd.to_datetime(post['created_at']) .strftime('%Y-%m-%d %H:%M:%5') }")
                else:
...
def dashboard_page():
    ...
    if users_success and posts_success:
        ...
        st.error("XFailed to load dashboard data")
